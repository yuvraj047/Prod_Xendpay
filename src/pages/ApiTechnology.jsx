      ...
      <section className="border-b border-subtle">
        <div className="max-w-content mx-auto px-5 md:px-8 py-14 md:py-16">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {API_STEPS.map((s, i) => (
              <div key={s.title} className="bg-surface border border-subtle rounded-2xl p-6">
                <Stop index={i + 1} color={i % 2 === 0 ? "amber" : "teal"} />
                <p className="mt-4 font-display font-semibold text-lg text-basetext">{s.title}</p>
                <p className="mt-1.5 text-sm text-muted font-body leading-relaxed">{s.copy}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-surfacemuted/40">
        <div className="max-w-content mx-auto px-5 md:px-8 py-16 md:py-20 grid md:grid-cols-2 gap-12">
          <div>
            ...
            <p className="mt-5 text-muted font-body leading-relaxed">
            ...
            <Link
              to="/contact"
              className="mt-8 inline-flex items-center gap-2 bg-amber text-ink font-body text-sm font-medium px-6 py-3.5 rounded-full hover:brightness-110 transition-all"
            >
              Request API Access →
            </Link>
          </div>
          ...
          <div className="bg-surface border border-subtle rounded-2xl p-8">
            <p className="eyebrow text-muted mb-5">Integration Support</p>
            <ul className="flex flex-col gap-3">
              {INTEGRATION_SUPPORT.map((item) => (
                <li key={item} className="flex items-center gap-3 text-sm font-body text-basetext">
